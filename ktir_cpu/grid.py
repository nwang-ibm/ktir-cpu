...
class GridExecutor:
    ...
    def execute_with_communication(self, execute_fn, max_rounds=10, *args, **kwargs):
        ...
        NOTE: When RingTransferBackend is introduced, this method will need
        access to the ring backend (via ExecutionEnv or a direct parameter)
        to drive message delivery between rounds — e.g. calling
        ring_backend.deliver_all() and checking for pending messages before
        deciding whether to continue or raise on non-convergence.  The
        coroutine-based scheduler will also need to resume suspended cores
        here after their send/recv has been satisfied.
        """
